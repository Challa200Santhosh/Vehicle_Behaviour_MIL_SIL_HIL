 ...
 *
 * SIL generated interface for code: "Vehicel_Behavior_Modeling"
 *
 */

#include "Vehicel_Behavior_Modeling.h"
#include "Vehicel_Behavior_Modeling_private.h"
...
XIL_INTERFACE_ERROR_CODE xilInitialize(uint32_T xilFcnId)
{
  ...
  /* Single In-the-Loop Component */
  if (xilFcnId == 0) {
    Vehicel_Behavior_Modeling_initialize();
  } else {
...
XIL_INTERFACE_ERROR_CODE xilOutput(uint32_T xilFcnId, uint32_T xilTID)
{
  ...
  switch (xilTID) {
   case 1:
    Vehicel_Behavior_Modeling_step();
    break;
...
XIL_INTERFACE_ERROR_CODE xilTerminate(uint32_T xilFcnId)
{
  ...
  Vehicel_Behavior_Modeling_terminate();
